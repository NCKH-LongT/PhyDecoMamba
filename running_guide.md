...
```bash
python src/training/train.py --config configs/snano.yaml --model Mamba1-Hybrid
```
...
```bash
python src/training/eval.py --config configs/snano.yaml --model_type Mamba1-Hybrid --model_path results/models/mamba1_hybrid_snano_best.pth
```
...
```bash
python src/training/eval.py --config configs/snano.yaml --models LSTM,ModernTCN,PatchTST,SimpleMamba,Mamba1-Hybrid --models_dir results/models
```
...
1. **Vẽ đồ thị xu hướng lỗi toàn bộ vòng đời (Full Life-cycle Trend)**:
   ```bash
   python src/evaluation/visualize_full_lifecycle.py --model_path results/models/mamba1_hybrid_snano_best.pth --model_type Mamba1-Hybrid --config configs/snano.yaml
   ```
2. **Kiểm tra lát cắt một file dữ liệu cụ thể (Single File Visualization)**:
   ```bash
   python src/evaluation/visualize_file.py --file data_B02_M1100.pt --model_path results/models/mamba1_hybrid_snano_best.pth --model_type Mamba1-Hybrid --config configs/snano.yaml
   ```
3. **Trực quan hóa xu hướng suy giảm trên tập test**:
   ```bash
   python src/evaluation/visualize_trend.py --model_path results/models/mamba1_hybrid_snano_best.pth --model_type Mamba1-Hybrid --config configs/snano.yaml
   ```
