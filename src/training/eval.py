...
def main():
    ...
    parser.add_argument("--model_type", type=str, default="Mamba1-Hybrid", help="Model type (e.g., Mamba1-Hybrid, SimpleMamba, LSTM, ModernTCN, PatchTST).")
    parser.add_argument("--batch_size", type=int, help="Override batch_size from config")
    parser.add_argument("--models", type=str, default=None, help="Comma-separated list of models to evaluate (e.g., 'LSTM,ModernTCN,PatchTST,SimpleMamba,Mamba1-Hybrid' or 'all').")
    ...
    def initialize_model(model_type):
        if model_type == "Mamba1-Hybrid":
            ...
            sm_d_model = config['model'].get('mamba_d_model', 64)
            return SimpleMamba({
                'model': {
                    'mamba_version': sm_version,
                    'mamba_d_model': sm_d_model,
            ...
            print(f"Initializing LSTM with parameters: hidden_dim=122, num_layers=3, horizon={horizon}")
            return LSTMForecaster(input_dim=2, hidden_dim=122, num_layers=3, horizon=horizon)
        elif model_type == "ModernTCN":
            print(f"Initializing ModernTCN with parameters: d_model=144, num_layers=3, kernel_size=17, horizon={horizon}")
            return ModernTCNForecaster(input_dim=2, d_model=144, num_layers=3, kernel_size=17, horizon=horizon)
        elif model_type == "PatchTST":
            print(f"Initializing PatchTST with parameters: d_model=128, nhead=16, num_layers=3, horizon={horizon}, lookback={lookback}, patch_size=16, stride=8")
    ...
    if args.models:
        if args.models == "all":
            models_to_eval = ["LSTM", "ModernTCN", "PatchTST", "SimpleMamba", "Mamba1-Hybrid"]
        else:
